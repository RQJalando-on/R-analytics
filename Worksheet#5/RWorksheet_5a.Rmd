...
name <- name[!grepl("Top 250 TV Shows|IMDb Charts|Recently viewed|More to explore", name, ignore.case = TRUE)]
name
```
```{r}
...
```
```{r}
date <- page2 %>% html_nodes(".sc-300a8231-7.eaXxft.cli-title-metadata-item") %>%  
  html_text()
date 

```
```{r}
user_rating <- page2 %>% html_nodes(".cli-ratings-container") %>%  
  html_text()
user_rating
```

```{r}
user_review = page %>% html_nodes(".ipc-btn__review") %>% 
  html_text()
user_review
```
```{r}
...
>>>>>>> 09d6de30edede901111839663577e81e36a16b7e
